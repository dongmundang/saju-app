...
const CLAUDE_MAX_TOKENS = parseInt(process.env.CLAUDE_MAX_TOKENS || '16000', 10);
...
- 차별화가 가장 중요한 원칙입니다. 각 항목(###)은 반드시 첫 1~2문장 안에 이 사람의 사주에만 있는 구체적 값(실제 일간 글자, 실제 오행 개수·비율, 실제 신살 이름, 실제 대운 나이 구간, 실제 십신 조합 등)을 직접 언급하며 시작하세요. "보통", "대체로", "~한 경향이 있는 분입니다" 같은 표현만으로 문단 전체를 채우지 마세요 — 그 경향이 왜 이 사주에서 나오는지(어떤 글자·어떤 오행 조합 때문인지)를 반드시 먼저 밝히고 나서 풀어 씁니다. 다른 사주였다면 이 문장이 그대로 쓰일 수 있는지 스스로 점검하고, 그렇다면 더 구체적으로 고쳐 씁니다.
...
- 일간의 한자는 천간 10자(갑甲·을乙·병丙·정丁·무戊·기己·경庚·신辛·임壬·계癸) 중 하나이며, 데이터의 "일간(본인): 을(乙)" 형식에 그대로 적혀 있습니다. "음"·"양"(음양)은 글자가 아니라 그 일간의 성질이므로, 절대 한자(陰·陽)로 바꿔 쓰거나 일간의 한자인 것처럼 쓰지 않습니다.
- 운명을 단정하지 않고 경향·가능성으로 말합니다. 겁주지 않고, 나쁜 부분도 대비하고 활용하는 방향으로 안내합니다.
- 각 항목을 충분히(대개 6~7문장) 풀어서 씁니다. 항목 제목(###)은 지시된 그대로 씁니다. 각 항목마다 실생활에서 있을 법한 구체적인 상황이나 장면(예: 직장에서 이런 순간에 이런 식으로 드러난다, 이런 성향 때문에 이런 선택을 하기 쉽다 등)을 최소 1개 이상 넣어 추상적인 설명에 머물지 않고 손에 잡히게 풀어 씁니다.`;
...
function sanitize(text) {
  if (!text) return text;
  let t = text;
  // thinking 비활성화 시 간혹 새어나오는 내부 태그 제거
  t = t.replace(/<\/?thinking>/gi, '');
  // 자주 나오는 잘못된 한자 교정
  t = t.replace(/驛馬殺|逆馬殺/g, '역마살');
  // 일간을 음양 성질(陰·陽)로 잘못 표기 → 올바른 천간 한자로 교정
  //  "을(陰)", "을(陰 목)", "을(陰, 목)", "을陰" 등 모두 처리
  const GAN_HAN = { '갑': '甲', '을': '乙', '병': '丙', '정': '丁', '무': '戊', '기': '己', '경': '庚', '신': '辛', '임': '壬', '계': '癸' };
  // 괄호 형태: 을(陰) / 을(陰 목) / 을(陰, 목(木))  → 을(乙) / 을(乙 목) ...
  t = t.replace(/([갑을병정무기경신임계])\s*[（(]\s*[陰陽][\s,，、]*([목화토금수])?(?:\s*[（(][木火土金水][）)])?\s*[）)]/g,
    (m, ko, oh) => oh ? `${ko}(${GAN_HAN[ko]} ${oh})` : `${ko}(${GAN_HAN[ko]})`);
  // 괄호 없는 형태: "일간이 을 陰", "을陰" (뒤에 다른 음양글자 없을 때)
  t = t.replace(/(일간[^.。\n]{0,14}?[은는이가]\s*[갑을병정무기경신임계])\s*[陰陽](?![陽陰])/g, '$1');
  t = t.replace(/([갑을병정무기경신임계])\s*[陰陽](?=\s*[목화토금수])/g, '$1 ');
...
// ── 주제별 풀이 ──
function buildPrompt(topic, sajuText, extra, age) {
  const isMinor = typeof age === 'number' && age < 19;
  const sections = {
    '총운': [
      '### 한눈에 보는 요약\n이 사주의 핵심을 3줄로. (1줄: 타고난 기질 / 2줄: 가장 큰 강점 / 3줄: 지금 시기 한마디)',
      '### 타고난 성격과 기질\n일간(본인)과 월지, 십신 구성으로 본 성격. 겉으로 보이는 모습과 속마음이 어떻게 다른지. 신강/신약이 성격에 주는 색깔. (6~7문장)',
      '### 대인관계와 일하는 방식\n비겁·식상·관성의 배치로 본 사람들과 어울리는 방식. 조직 생활이 맞는지 독립이 맞는지, 앞에 나서는 형인지 뒤에서 받치는 형인지. (4~5문장)',
      '### 타고난 강점과 재능\n이 사주가 가진 무기. 특히 신살(도화·문창귀인·역마·화개·천을귀인 등)이 있으면 그것이 뜻하는 재능과 살릴 방향. (6~7문장)',
      '### 약점과 인생의 과제\n신약/신강의 그늘, 부족하거나 지나친 오행이 만드는 어려움, 삶에서 반복되기 쉬운 패턴. 지지 형충이 있으면 그 영향. (4~5문장)',
      '### 인생의 큰 흐름 (대운)\n제공된 대운 흐름을 보고 어느 시기가 순풍이고 어느 시기가 역풍인지, 지금 대운(현재 표시된 것)의 의미, 다음 대운은 언제 어떻게 바뀌는지, 전성기로 볼 만한 시기. (7~8문장)',
      '### 상담가의 조언\n이 분이 삶에서 붙잡으면 좋은 것을 구체적으로 3가지. 용신 오행을 살리는 방향(잘 맞는 직업 분야, 도움이 되는 색·방향·활동)을 실제로 제시. (4~5문장)'
    ],
    '연애': isMinor ? [
      '### 한눈에 보는 요약\n또래 관계·친구를 사귀는 성향의 핵심을 3줄로.',
      '### 친구를 사귀는 방식\n타고난 대인관계 성향, 마음을 여는 방식, 또래에게 어떻게 비치는지. 도화살·홍염 등이 있으면 사람을 끄는 매력으로 풀이. (6~7문장)',
      '### 잘 맞는 친구와 관계의 특징\n비겁·식상 등으로 본 잘 맞는 친구 유형과 관계 맺는 방식. (6~7문장)',
      '### 자라면서 관계에서 주의할 점\n친구·또래 관계에서 반복되기 쉬운 패턴과 그 이유, 부드럽게. 연애·결혼·이성 관계 언급은 하지 말 것. (4~5문장)',
      '### 상담가의 조언\n좋은 친구 관계를 맺도록 돕는 구체적 조언 3가지. (4~5문장)'
    ] : [
      '### 한눈에 보는 요약\n연애·결혼운의 핵심을 3줄로.',
      '### 연애 성향과 매력\n타고난 연애 스타일, 사랑에 빠지는 방식, 이성에게 어떻게 보이는지. 도화살·홍염 등이 있으면 그 영향. (6~7문장)',
      '### 어떤 인연과 만나기 쉬운가\n남자는 재성(정재·편재), 여자는 관성(정관·편관)으로 본 배우자·애인의 모습과 성향. 배우자궁(일지)의 상태가 뜻하는 것. (6~7문장)',
      '### 결혼운과 시기\n결혼 인연이 오기 쉬운 흐름, 대운·세운에서 배우자 십신이 강해지는 때. (4~5문장)',
      '### 연애에서 반복되기 쉬운 어려움\n이 사주가 관계에서 부딪히기 쉬운 패턴과 그 이유. 일지 형충이 있으면 그 영향. 겁주지 말고 대비하는 쪽으로. (4~5문장)',
      '### 상담가의 조언\n좋은 인연을 만나고 관계를 지키기 위한 구체적 조언 3가지. (4~5문장)'
    ],
    '재물': isMinor ? [
      '### 한눈에 보는 요약\n타고난 재물 그릇과 적성의 핵심을 3줄로.',
      '### 타고난 재물 그릇\n재성의 유무와 위치로 본, 이 사람이 자라서 가지게 될 돈에 대한 태도와 그릇. 지금 돈을 벌거나 관리한다고 서술하지 말 것. (6~7문장)',
      '### 앞으로 잘 맞을 분야\n십신·오행·신살로 본 적성과 자라서 살릴 수 있는 재능 분야를 구체적으로. (6~7문장)',
      '### 자라면서 도움이 되는 습관\n용신 오행을 살리는 생활 습관이나 부모가 지금부터 도와줄 수 있는 방향. (4~5문장)',
      '### 상담가의 조언\n아이의 재능과 재물 그릇을 잘 키워주기 위해 부모님께 드리는 조언 3가지. (4~5문장)'
    ] : [
      '### 한눈에 보는 요약\n재물·직업운의 핵심을 3줄로.',
      '### 돈이 들어오고 나가는 방식\n재성(정재·편재)의 유무와 위치로 본 돈 버는 스타일 — 안정적인 월급형인지, 크게 벌고 크게 쓰는 사업형인지, 큰돈과 인연이 있는지. 식상생재(만들어서 파는 구조)가 되는지. (6~7문장)',
      '### 잘 맞는 직업과 일하는 방식\n십신·오행·신살로 본 적성 분야를 구체적으로. 관성이 좋으면 조직·전문직, 식상이 좋으면 창작·기술·영업 등. (6~7문장)',
      '### 재물운의 큰 흐름\n대운·세운에서 재성·식상 운이 들어오는 시기가 재물이 불어나기 좋은 때. 반대로 조심할 시기. (4~5문장)',
      '### 돈 관리에서 조심할 점\n재물이 새기 쉬운 패턴(겁재, 편재 과다 등), 투자·보증·동업에서 주의점. (4~5문장)',
      '### 상담가의 조언\n재물을 모으고 지키기 위한 구체적 조언 3가지. (4~5문장)'
    ],
    '건강': [
      '### 한눈에 보는 요약\n체질과 건강 흐름의 핵심을 3줄로.',
      '### 타고난 체질\n오행 균형으로 본 몸의 강한 곳과 약한 곳. 일간 오행과 신강/신약이 체력·회복력에 주는 영향. (4~5문장)',
      '### 특히 신경 써야 할 부위\n부족하거나 지나친 오행에 해당하는 장기·계통(목=간담·눈·근육, 화=심장·혈압·수면, 토=위장·소화, 금=폐·기관지·피부, 수=신장·방광·뼈), 나타나기 쉬운 증상. (6~7문장)',
      '### 조심할 시기\n대운·세운에서 기신 오행이 강해지거나 지지 충이 걸리는 때 건강에 더 유의. 삼재 기간이 있으면 무리하지 않도록. (4~5문장)',
      '### 생활 습관 제안\n용신 오행을 돕는 방향으로 음식·운동·수면·환경을 구체적으로. (4~5문장)',
      '### 상담가의 조언\n진단이 아닌 참고임을 전제로, 건강을 지키는 큰 원칙 2~3가지. (4~5문장)'
    ]
  };
...
function buildDailyPrompt(sajuText, iljin, age) {
  const isMinor = typeof age === 'number' && age < 19;
  const ageNote = isMinor
    ? `\n\n※ 이 손님은 현재 만 ${age}세로 미성년자(아동/청소년)입니다. 연애·직장·돈벌이 등 성인의 삶을 전제로 한 표현(연인, 직장, 투자 등)은 쓰지 말고, 학업·가족·또래 관계·컨디션 등 이 나이에 맞는 내용으로만 풀이하세요.`
    : '';
  return `아래는 한 손님(만 ${age != null ? age + '세' : '나이 미상'})의 사주와 오늘 날짜의 일진입니다.${ageNote}
...
function buildLuckPrompt(sajuText, luck, age) {
  const isMinor = typeof age === 'number' && age < 19;
  const ageNote = isMinor
    ? `\n\n※ 이 사람은 현재 만 ${age}세로 미성년자(아동/청소년)입니다. "사회에서 인정받는 시기", "책임과 도전", "직장", "연애·애정" 같은 성인 사회생활을 전제로 한 표현은 절대 쓰지 마세요. 이 나이에 맞게 건강·기질·가족관계·또래관계·호기심/학습 같은 주제로만 풀이하세요.`
    : '';
  const fieldsLine = isMinor
    ? '건강 / 기질·성격 변화 / 가족·또래 관계 / 호기심과 배움 — 각 항목 2~3문장씩.'
    : '재물 / 직장·일 / 인간관계 / 건강 / 애정 — 각 항목 2~3문장씩.';
  return `아래는 한 손님(만 ${age != null ? age + '세' : '나이 미상'})의 사주와 현재 시점의 세운(올해)·월운(이번 달)입니다.${ageNote}
...
### 올해 전체 흐름
세운의 십신·오행이 본인 사주(특히 용신/기신)에 어떻게 작용하는지, 올해의 큰 주제. (6~7문장)

### 이번 달 포인트
월운을 반영해 지금 한 달 신경 쓸 일과 기회. (4~5문장)

### 분야별 전망
${fieldsLine.replace(/각 항목 2~3문장씩/, '각 항목 4~5문장씩')}
...
### 성격 궁합
두 사람의 십신·신강신약을 비교해 잘 맞는 점과 부딪힐 수 있는 점. (4~5문장)
